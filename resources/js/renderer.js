...
function updateStatus(status) {
  if (status.simConnected !== undefined) {
    appState.simConnected = status.simConnected;
    elements.simStatus.textContent = status.simConnected ? 'Connected' : 'Disconnected';
  }
  
  if (status.serverConnected !== undefined) {
    appState.serverConnected = status.serverConnected;
    elements.serverStatus.textContent = status.serverConnected ? 'Connected' : 'Disconnected';
  }
  
  if (status.telemetryRunning !== undefined) {
    appState.telemetryRunning = status.telemetryRunning;
    elements.startTelemetry.disabled = status.telemetryRunning;
    elements.stopTelemetry.disabled = !status.telemetryRunning;
    
    if (status.telemetryRunning) {
  ...
  const isConnected = appState.simConnected && appState.serverConnected;
  elements.statusIcon.className = isConnected ? 'status-icon connected' : 'status-icon';
  elements.statusText.textContent = isConnected ? 'Connected' : 'Disconnected';
  
  // Log status change
  addLogEntry(`Status update: ${JSON.stringify(status)}`);
}
