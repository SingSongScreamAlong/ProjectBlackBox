...
class WebSocketService {
  ...
  onCoaching(callback: EventCallback<CoachingInsight[]>): () => void {
    return this.on('coaching', callback);
  }
  
  onSkillAnalysis(callback: EventCallback<DriverSkillAnalysis>): () => void {
   ...
   * @returns True if the message was sent successfully, false otherwise
   */
  sendTeamMessage(content: string, priority: 'normal' | 'high' | 'critical' = 'normal', senderId: string = 'system', senderName: string = 'System'): boolean {
    const message = {
      message: {
        id: uuidv4(),
        senderId,
