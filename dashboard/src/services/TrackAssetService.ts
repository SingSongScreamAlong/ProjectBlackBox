    ...
    async initialize() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // 404 or other error - this is expected if sync-tracks hasn't been run
                console.info('TrackAssetService: tracks.json not available (sync-tracks not run). Using fallback tracks only.');
            }
        } catch (error) {
            // Silently handle - fallback tracks will be used
            console.info('TrackAssetService: Using fallback tracks only.');
        }
    }

    /**
     * Check if dynamic track assets are available
     */
    hasDynamicAssets(): boolean {
        return this.isInitialized && Object.keys(this.trackIndex).length > 0;
    }
