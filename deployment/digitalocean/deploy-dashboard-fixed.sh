#!/bin/bash
# BlackBox Dashboard Deployment Script (Fixed Version)
# This script deploys the simplified dashboard to your DigitalOcean droplet with macOS compatibility
...
echo "🚀 BlackBox Dashboard Deployment (FIXED VERSION)"
echo "==============================="
...
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 $SSH_USER@$DROPLET_IP "echo SSH connection successful" || { 
  echo "SSH connection failed or timed out. Please check:"
  echo "1. Is the droplet IP ($DROPLET_IP) correct?"
  echo "2. Do you have SSH access to the droplet?"
  echo "3. Is your SSH key properly configured?"
  exit 1
}

# Create a temporary directory for the build
echo "Creating remote directory..."
ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 $SSH_USER@$DROPLET_IP "mkdir -p $REMOTE_DIR/dashboard-build" || { 
...
scp -o ConnectTimeout=5 -r build/* $SSH_USER@$DROPLET_IP:$REMOTE_DIR/dashboard-build/ || { 
...
scp -o ConnectTimeout=5 Dockerfile nginx.conf $SSH_USER@$DROPLET_IP:$REMOTE_DIR/ || { 
  echo "Failed to copy Dockerfile and nginx.conf"
  exit 1
}
